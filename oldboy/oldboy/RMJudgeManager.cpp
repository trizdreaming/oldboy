...
void CRMjudgeManager::PrintScore( PlayerNumber player, JudgeType judgeType )
{
	...
	wchar_t		*playerScoreLabelName; //SM9: 별로 좋지 못한 습관. 반드시 default로 초기화 할 것. 이 경우는 LABEL_NAME_P1_SCORE 같은걸로 해놓고,
	// 플레이어 2일 경우만 다른걸로 교체하는 식으로 해라.. 괜히 아래의 스위치 문을 쓸 필요 없이 if 한번이면 된다.
	// 그리고 LABEL_NAME_P1_SCORE LABEL_NAME_P1_COMBO 같은 애들을 이렇게 포인터 대입해서 쓸 필요 없이
	// 저기 아래 CreateLabel 할 때 ( player1 ? LABEL_NAME_P1_SCORE : LABEL_NAME_P2_SCORE) 이런식으로 쓰는게 더 깔끔하겠지?
