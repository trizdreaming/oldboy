...
CRMmainLoop::CRMmainLoop(void):
	...
	m_Hwnd(NULL),
	m_PlayMusicName("") //SM9: 스트링 같은 애들은 이렇게 초기화 할 필요 없다. 자체 생성자에서 초기화 해줌.
{
...
void CRMmainLoop::RunMessageLoop()
{
	...
	//===================================================================
	// 음악 데이터를 불러온다.
	//SM9: 이런거는 에러처리 잘 해주어야 한다. 만일 음악 데이터 불러오는게 실패하더라도 문제 없는지? 
	LoadMusicData();

	// test 음악선택하기
	//SM9: 위에서 LoadMusicData가 실패했다면 여기서 포인터 참조 에러 나겠지...
	...
	while ( true )
	{
		...
		else
		{
			...
			//	printConsole("FPS : %d \n", fps);
				...
				swprintf_s(testChar, L"FPS : %d ", fps); //SM9: fps 같은 지역 변수는 가장 처음 쓰이는 곳 바로 윗쪽에 두는게 코드 보기 편하다.
...
void CRMmainLoop::LoadMusicData()
{
	...
	HANDLE hFind = FindFirstFileA( MUSIC_FOLDER_SEARCH, &findFileData ); //SM9: ASCII 전용 함수를 쓰는 이유는? 라이브러리 호환성 때문에 그런가?
...
HRESULT CRMmainLoop::CreateObject()
{
	...
	for ( int i = 0 ; i < 20 ; ++ i ) //SM9: 20?? DEFINE같은거로 빼고, 네이밍 잘 해야 의미가 명백해진다.
	{
